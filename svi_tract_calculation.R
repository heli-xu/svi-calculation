...
var_list <- readRDS("data/census_variables_2020.rds") %>% 
...
save(ri_ct_raw, file = "data/ri_tract_2020_raw.rds")
...
ri_ct_e_ep <- 
  ...
  drop_na() #added after the initial check 

#sanity check
RI_2020_ct <- read_csv("download/RI_tract_2020.csv") %>% 
  rename(GEOID = FIPS)
...
#adding a drop_na() to remove rows(tracts) with all NAs
...
#check
test <- ri_ct_epl %>% 
  filter(svi_var == "EP_POV150") %>% 
  select(GEOID, svi_var, value, EPL_var)

cdc2 <- RI_2020_ct %>% 
  select(GEOID, EPL_POV150)
