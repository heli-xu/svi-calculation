...
svi_var <- read_xlsx("download/2016svi_dictionary.xlsx") %>% 
...
svi_var <- svi_var %>% 
  rename(x2016_table_field_calculation = table_field_calculation)
#edits from 2016 
...
svi_var_clean <- svi_var %>% 
  #filter(!x2016_table_field_calculation_if_changed %in% pages) %>% 
  mutate(x2016_variable_name = replace_na(x2016_variable_name, "x")) %>% 
  filter(!str_starts(x2016_variable_name,"M")) 

#table with theme, variable, calculation
var_cal <- svi_var_clean %>% 
  select(x2016_variable_name, theme, x2016_table_field_calculation) %>% 
  drop_na(x2016_table_field_calculation) %>% 
  filter(!str_detect(x2016_table_field_calculation,"\\^"))
...
var_cal <- var_cal %>% 
  mutate(
    x2016_table_field_calculation = str_replace_all(x2016_table_field_calculation, "\r\n",""),
    #pdf to excel introduce line breaks, not visible in view- use $ index to see 
    census_var = str_replace_all(x2016_table_field_calculation,
                                      "[^[:alnum:][:blank:]_]",
                                      " ")) 
...
var_cal2 <- 
  var_cal %>%
  filter(str_detect(x2016_variable_name, paste(var_prefix, collapse = "|"))) %>% 
  mutate(theme = case_when(
    x2016_variable_name%in%c("E_TOTPOP","E_HU","E_HH") ~ 0,  #must be same class
...
saveRDS(var_cal_eep, file = "data/variable_e_ep_calculation_2016.rds")
...
saveRDS(var_list, "data/census_variables_2016.rds")  
